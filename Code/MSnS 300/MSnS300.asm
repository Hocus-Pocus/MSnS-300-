...
;***********************************************************************************************
     ...
     lda     #$0F            ; Load accumulator with decimal 15 (300RPM)
                             ;NOTE! THIS IS MY ERROR, IT'S NOT RPM, BUT RATHER THEH NUMBER OF ELEMENTS -1.SHOULD BE 3.
     sta     tmp3            ; Copy to TMP3 variable
...
;FIND_TOTAL_A:
     lda     tmp6            ; Load accumulator with value in tmp6(result of lininterp TPSDOT lookup)(taecor)
     tax                     ; (A)->X Transfer to index register Lo byte
     lda     tmp14           ; Load accumulator with value in tmp14 (ACCELMULT)(ColdMul)
     mul                     ; (X)x(A)->X:A Multiply tmp6 by tmp14(TPSDOT lookup x ACCELMULT)(taecor x ColdMul)
     pshx                    ; Push index register Lo onto stack (result of mul Hi byte)
     pulh                    ; Pull index register L to index register Hi(result Hi byte ->H
     ldx     #$64            ; Load index register low with decimal 100
     div                     ; Divide A<-(H:A)/(X);H<-Remainder(taecor x ColdMul)/100
     bcs     UPPER_RAIL_AE   ; If the C bit of CCR is set, branch to UPPER_RAIL_AE:
                             ; C is set with quotient overflow (qoutient>$FF) or divide by zero
     psha                    ; Push accumulator onto stack (result of div)
     pshh                    ; Push index register high onto stack (remainder of div)
     pula                    ; Pull index register high into accumulator(remainder of div -> A)
     cmp     #$32            ; Compare accumulator with decimal 50
     ble     NDA1            ; If (A) is less than or equal to (M), branch to NDA1:(remainder of div <= 50)
     pula                    ; Pull value from stack to accumulator(result of div -> A)
...
ADD_TO_AE:
     add     tmp13            ; (A)+(M)->A Add value in accumulator with value in tmp13(Add on the amount
     ...
     blo     TAE_CHK_TIME     ; If (A) is lower than (M), branch to TAE_CHK_TIME:
     lda     tmp6             ; Load accumulator with value in tmp6
...
TAE_CHK_TIME:
     ...
     blo     TDE_DONE                ; If (A) is lower than (M), branch to TDE_DONE:
...
TDE_DONE:
     bclr    clock100,inputs   ; Clear "clock100" bit of "inputs" variable
                               ;NOTE! THIS COULD BE AN ERROR, NOT SURE IF IT DOES ANY HARM OR NOT
